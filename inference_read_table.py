...
class SavePredictor(Predictor):
    ...
    def save_prediction(self, image: np.ndarray, dpi: int, input_path: Path):
        ...
        for i in range(boxes.shape[0]):
            ...
            if class_id == 0:
                continue  # TODO compare the predicted cells with the cells inferred from the rows and columns
                bboxes["cells"].append(xyxy)
            elif class_id == 1:
                bboxes["rows"].append(xyxy)
            elif class_id == 2:
                bboxes["cols"].append(xyxy)
